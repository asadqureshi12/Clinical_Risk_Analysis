...
| `risk_level_distribution.png` | Bar chart: Risk Level Distribution |
| `encounters_vs_age_by_risk.png` | Scatter plot: Encounters vs Age by Risk Level |
| `avg_medications_age_group.png` | Bar chart: Average Medications per Age Group |
| `avg_encounters_conditions.png` | Bar chart: Average Encounters by Number of Conditions |
...
Patients are then categorized into risk tiers based on their total score:
...
The cohort shows a meaningful spread across all three risk tiers, indicating a population with varied clinical complexity.
...
![Encounters vs Age by Risk Level](encounters_vs_age_by_risk.png)  
![Average Medications per Age Group](avg_medications_age_group.png)
...
Patients with more diagnosed conditions also show substantially higher encounter rates:

![Average Encounters by Number of Conditions](avg_encounters_conditions.png)
...
The cohort is nearly evenly split: 52.0% female, 48.0% male, indicating no significant gender imbalance.
...
For this exploratory analysis, values are retained to preserve the original dataset structure. In a production healthcare workflow, additional preprocessing such as outlier detection, medication reconciliation, or capping extreme values would typically be applied.
